...
_Impact:_ M1 in full, and the ownership test suite.
_Default if unanswered:_ **multi-user with self-registration**, full per-user isolation.
...
_Impact:_ M3, M4, and every reported total.
_Default if unanswered:_ **single base currency chosen at sign-up**, stored per row so the
...
_Why not Auth.js v5:_ it remains `5.0.0-beta.32`. _Why no OAuth:_ Google/Apple/GitHub
...
_record_ of decisions, not a quote terminal.
...
_possible_ but costs searchability and adds key-management burden. Flagged for a decision
...
## Discovered during M0/M1 — 2026-09-20

### D-14 — Rate limiting without a trusted proxy
...
ceilings (`networkScope` in `src/lib/security/rate-limit.ts`). The unattributed
ceiling caps runaway abuse rather than throttling individuals, because an
unattributable request cannot be throttled individually.

**Still owed:** per-IP limiting is only meaningful once a proxy that rewrites
`X-Forwarded-For` sits in front of the app and `TRUST_PROXY_HEADERS` is on.
That is a deployment requirement, not something code can assert.

### D-15 — Migration rollback

drizzle-kit generates forward migrations only. M0's acceptance criterion says a
migration "applies and rolls back"; only the forward half is verified.

**Options:** hand-written down-migrations kept alongside each forward file, or
a restore-from-backup policy that treats rollback as an operational procedure
rather than a code artifact. **Needs a decision before M3**, when the finance
schema starts holding data a bad migration could damage.

### D-16 — ESLint major version

Pinned to 9.39.5. ESLint 10 crashes `eslint-plugin-react`, which
`eslint-config-next` depends on. Revisit when that plugin supports ESLint 10;
purely a tooling pin with no effect on application behaviour.
