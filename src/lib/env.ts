...
const schema = z
  .object({
     ...
     * Email transport.
     *
     * "console" writes the message to the server log and sends nothing — the
     * honest default when no provider is configured. "resend" delivers through
     * Resend's HTTP API and needs RESEND_API_KEY. "none" disables sending.
     */
    EMAIL_TRANSPORT: z.enum(['console', 'resend', 'none']).default('console'),

    /**
     * The From address. With Resend this must be on a domain verified in the
     * Resend account; `onboarding@resend.dev` works without one, but only
     * delivers to the Resend account owner's own address.
     */
    EMAIL_FROM: z.string().default(emailFrom('no-reply@localhost')),

    /** Required when EMAIL_TRANSPORT is "resend". A credential: never logged. */
  ...
  .superRefine((env, ctx) => {
    // Choosing a provider without its credential would otherwise boot fine and
    // fail on the first send — as a user staring at a reset form that never
    // delivers. Refuse at startup instead.
